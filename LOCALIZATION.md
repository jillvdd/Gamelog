...
|-----|---------|---------|--------|---------|
| `about.developer` | 开发者 | Developer | 開発者 | Views/AboutView |
| `about.menu` | 关于我的游戏簿 | About My Gamelog | My Gamelog について | GameLogApp |
...
| `backup.autobackupRestoreConfirm` | 将用自动备份替换当前全部数据，确定继续吗？ | Restoring will replace all current data with the auto backup. Continue? | 現在の全データが自動バックアップに置き換えられます。続行しますか？ | Views/SettingsView |
| `backup.backupNow` | 立即备份 | Back Up Now | 今すぐバックアップ | Views/SettingsView |
| `backup.emptyRestoreMessage` | 当前库为空，但本地自动备份包含 %lld 款游戏。是否从自动备份恢复？（取消将保留空库） | Your library is empty, but the local auto backup contains %lld games. Restore from auto backup? (Cancel keeps the empty library) | 現在のライブラリは空ですが、自動バックアップには %lld 件のゲームがあります。自動バックアップから復元しますか？（キャンセルで空のままにします） | Support/AutoBackup |
| `backup.emptyRestoreTitle` | 检测到库为空 | Empty library detected | ライブラリが空です | Support/AutoBackup |
| `backup.export` | 导出备份… | Export Backup… | バックアップを書き出し… | Views/SettingsView |
...
| `backup.import` | 导入备份… | Import Backup… | バックアップを読み込み… | Views/SettingsView |
| `backup.importConfirm` | 导入将替换当前全部数据，确定继续吗？ | Importing will replace all current data. Continue? | 読み込むと現在の全データが置き換えられます。続行しますか？ | Views/SettingsView,Views/iOSRootView |
| `backup.importDone` | 备份已导入 | Backup imported | バックアップを読み込みました | Views/SettingsView |
| `backup.importFailed` | 导入失败，请检查文件是否为有效备份 | Import failed. Check that the file is a valid backup. | 読み込みに失敗しました。有効なバックアップか確認してください | Views/SettingsView |
| `backup.keepEmpty` | 保留空库 | Keep Empty | 空のままにする | Support/AutoBackup |
| `backup.lastBackup` | 上次备份：%@ | Last backup: %@ | 最終バックアップ：%@ | Views/SettingsView |
| `backup.noBackupYet` | 尚未备份 | No backup yet | バックアップはまだありません | Views/SettingsView |
...
| `backup.restoreFailed` | 恢复失败，自动备份文件可能已损坏 | Restore failed. The auto backup file may be corrupted. | 復元に失敗しました。自動バックアップファイルが壊れている可能性があります | Views/SettingsView |
...
| `completion.degree` | 通关程度 | Completion degree | クリア度 | Views/CompletionEditView,Views/GameEditView |
| `completion.delete` | 删除 | Delete | 削除 | Views/GameDetailView |
| `completion.edit` | 编辑 | Edit | 編集 | <未使用>*(动态/未命中)* |
| `completion.noDate` | 无通关日期 | No clear date | クリア日なし | Views/CompletionEditView,Views/GameEditView |
| `completion.noPlaytime` | 无时长 | No playtime | プレイ時間なし | Views/CompletionEditView,Views/GameEditView |
| `completion.notes` | 通关内容 | Completion notes | クリアメモ | <未使用>*(动态/未命中)* |
| `completion.notesPlaceholder` | 这次的游玩感受、趣事… | Thoughts, stories from this run… | 今回の感想・出来事… | <未使用>*(动态/未命中)* |
...
| `completion.playtimeFormat` | %.1f 小时 | %.1f h | %.1f 時間 | Views/GameDetailView |
| `completion.scores` | 六维评分 | Dimension Scores | 6次元スコア | Views/CompletionEditView,Views/GameEditView |
...
| key | 简体中文 | English | 日本語 | 使用位置 |
|-----|---------|---------|--------|---------|
| `copy.addImage` | 添加图片 | Add Images | 写真を追加 | Views/HoldingsView |
| `copy.addVersion` | 新增版本 | Add Edition | 版を追加 | Views/HoldingsView |
| `copy.count` | 数量 | Quantity | 数量 | Views/HoldingsView |
| `copy.deleteConfirm` | 确定删除版本「%@」？该版本的照片也会被删除。 | Delete edition \"%@\"? Photos for this edition will also be deleted. | 版「%@」を削除しますか？この版の写真も削除されます。 | Views/HoldingsView |
| `copy.deleteImageConfirm` | 确定删除这张照片吗？此操作无法撤销。 | Delete this photo? This action cannot be undone. | この写真を削除しますか？この操作は元に戻せません。 | Views/HoldingsView |
| `copy.noCopies` | 还没有持有记录 | No holdings yet | 所持記録がありません | Views/HoldingsView |
| `copy.rename` | 重命名版本 | Rename Edition | 版名を変更 | Views/HoldingsView |
| `copy.version` | 版本 | Edition | 版 | Views/HoldingsView |
| `copy.versionAuto` | 版本 %d | Edition %d | 版 %d | Views/HoldingsView |
| `copy.versionPlaceholder` | 如「日版初版」 | e.g. JP launch edition | 例「日本版初版」 | Views/HoldingsView |
...
| `cover.noGrids` | 未找到该游戏的封面 | No covers found for this game | このゲームのカバーは見つかりませんでした | Views/CoverSearchSheet |
| `cover.searchFailed` | 封面获取失败，请检查网络或 API Key | Couldn't fetch covers. Check your network or API key. | カバーを取得できませんでした。ネットワークか API Key を確認してください | Views/CoverSearchSheet |
| `cover.title` | 搜索封面 | Search Cover | カバーを検索 | Views/CoverSearchSheet |

## `crop.*` 组

| key | 简体中文 | English | 日本語 | 使用位置 |
|-----|---------|---------|--------|---------|
| `crop.titleAvatar` | 裁切头像 | Crop Avatar | アバターを切り抜く | Views/ImageCropView |
| `crop.titleIcon` | 裁切图标 | Crop Icon | アイコンを切り抜く | Views/ImageCropView |
...
| key | 简体中文 | English | 日本語 | 使用位置 |
|-----|---------|---------|--------|---------|
| `date.day` | 日 |  | 日 | Views/DateMenuPicker |
| `date.earlierYears` | 更早年份… | Earlier Years… | より前の年… | <未使用>*(动态/未命中)* |
| `date.year` | 年 |  | 年 | Views/DateMenuPicker |

## `delete.*` 组

| key | 简体中文 | English | 日本語 | 使用位置 |
|-----|---------|---------|--------|---------|
| `delete.confirmGame` | 确定删除「%@」？该游戏的所有通关记录也会被删除。 | Delete “%@”? All of its completions will be deleted too. | 「%@」を削除しますか？このゲームの全クリア記録も削除されます。 | Views/GameDetailView,Views/LibraryView |
...
| `game.completions` | 通关记录 | Completions | クリア記録 | Views/GameDetailView |
...
| `game.name` | 名称 | Name | 名称 | Views/GameEditView |
| `game.nameEn` | 英文名 | English name | 英語名 | Views/GameEditView |
| `game.nameJa` | 日文名 | Japanese name | 日本語名 | Views/GameEditView |
| `game.nameZh` | 中文名 | Chinese name | 中国語名 | Views/GameEditView |
...
| `game.reviewTitlePlaceholder` | 一句话评价（分享图会用到） | One-line verdict (shown on share card) | 一言レビュー（共有画像に表示） | Views/ReviewEditSheet,Views/GameEditView |
| `game.searchCover` | 搜索封面… | Search Cover… | カバーを検索… | Views/GameEditView |
| `game.status` | 状态 | Status | ステータス | Views/RootView,Views/iOSRootView,Views/LibraryView,Views/GameEditView |
| `game.statusHint` | 想玩/在玩/搁置/弃坑无需通关记录与评分，流转到「已通关」时再补。 | Lightweight states (Backlog, Playing…) need no completion records or scores — add them when you move to Completed. | プレイ予定/プレイ中などの軽量状態ではクリア記録やスコアは不要です。「クリア済み」にした時に記録します。 | Views/GameEditView |

## `group.*` 组

| key | 简体中文 | English | 日本語 | 使用位置 |
|-----|---------|---------|--------|---------|
| `group.avgScore` | 平均分 | Avg Score | 平均スコア | Views/StatsView,Views/StatsRankings,Views/GroupFooter,Share/ShareCardView |
| `group.completionCount` | 通关数 | Completions | クリア数 | Share/ShareCardView |
| `group.deleteConfirm` | 确定删除分组「%@」？分组中的游戏不会被删除。 | Delete group “%@”? Games in it will not be deleted. | 「%@」を削除しますか？グループ内のゲームは削除されません。 | Views/iOSRootView,Views/RootView |
...
| `group.manage` | 管理分组 | Manage Groups | グループ管理 | Views/iOSRootView |
| `group.memberCount` | %d 款游戏 | %d games | %d 個のゲーム | Views/GroupGamePickerView |
| `group.name` | 分组名称 | Group name | グループ名 | Views/GameCardView,Views/RootView |
| `group.nameExists` | 该分组名已存在 | A group with this name already exists | 同じ名前のグループが既にあります | Views/RootView,Views/GameCardView |
...
| `group.review` | 分组评价 | Group Review | グループ評価 | Views/GroupFooter |
| `group.reviewEdit` | 编辑分组评价 | Edit Group Review | グループ評価を編集 | Views/GroupFooter |
| `group.reviewEmpty` | 还没有评价 | No review yet | 評価はまだありません | Views/GroupFooter |
...
| key | 简体中文 | English | 日本語 | 使用位置 |
|-----|---------|---------|--------|---------|
| `image.cameraUnavailable` | 当前设备不支持拍照 | Camera is not available on this device | この端末では撮影できません | Support/ImageSourcePicker |
| `image.fromFiles` | 从文件选择 | Choose from Files | ファイルから選択 | Support/ImageSourcePicker |
| `image.fromPhotos` | 从相册选择 | Choose from Photos | 写真ライブラリから選択 | Support/ImageSourcePicker |
| `image.pickSource` | 添加图片 | Add Image | 画像を追加 | Support/ImageSourcePicker |
| `image.takePhoto` | 拍照 | Take Photo | 撮影 | Support/ImageSourcePicker |
...
| `library.filter` | 筛选 | Filter | フィルター | Views/iOSRootView |
| `library.filterGroup` | 分组 | Group | グループ | Views/iOSRootView |
| `library.filterPlatform` | 平台筛选 | Platform | プラットフォーム | Views/iOSRootView,Views/LibraryView,Views/GroupGamePickerView,Views/StatsRankings |
| `library.gridView` | 网格视图 | Grid view | グリッド表示 | Views/LibraryView |
| `library.listView` | 列表视图 | List view | リスト表示 | Views/LibraryView |
| `library.noGames` | 还没有游戏，点击右上角「新建游戏」开始记录 | No games yet. Click “Add Game” in the top-right to start. | まだゲームがありません。右上の「ゲームを追加」から記録を始めましょう | Views/LibraryView |
...
| `library.sortByScoreAsc` | 平均分（从低到高） | Average score (low to high) | 平均点（低い順） | Views/LibraryView |
| `library.sortByScoreDesc` | 平均分（从高到低） | Average score (high to low) | 平均点（高い順） | Views/LibraryView |
...
| key | 简体中文 | English | 日本語 | 使用位置 |
|-----|---------|---------|--------|---------|
| `review.bodyHint` | 支持 Markdown：标题用 #，加粗用 **文字**，斜体用 *文字*，列表用 - 项 | Markdown supported: # heading, **bold**, *italic*, - list item | Markdown 対応：# 見出し、**太字**、*斜体*、- リスト | Views/GameEditView |
...
| key | 简体中文 | English | 日本語 | 使用位置 |
|-----|---------|---------|--------|---------|
| `score.average` | 平均分 | Average | 平均 | Views/GameDetailView |
...
| `settings.autoBackupHint` | 每次数据改动后自动在本地保存完整备份，可在设置中随时恢复。 | Automatically saves a full backup locally whenever your data changes. Restore anytime from Settings. | データが変更されるたびに完全なバックアップを自動保存します。設定からいつでも復元できます。 | Views/SettingsView |
| `settings.autoMatchCover` | 自动匹配封面 | Auto-fill Cover | カバー自動取得 | Views/SettingsView |
| `settings.autoMatchCoverHint` | 输入游戏名后，自动搜索 SteamGridDB 并填入第一张封面（需在下方配置 API Key）。 | When you type a game name, automatically searches SteamGridDB and sets the first cover found (requires an API key below). | ゲーム名を入力すると SteamGridDB を検索し、最初のカバーを自動設定します（下の API Key の設定が必要です）。 | Views/SettingsView |
...
| `settings.cacheClearConfirm` | 清除缓存？缓存仅为可自动重建的临时数据（封面搜索下载、图片解码缓存、临时文件），不会影响游戏数据与备份。 | Clear caches? Caches are temporary data that rebuilds automatically (cover search downloads, image decode caches, temporary files). Your game data and backups are not affected. | キャッシュを削除しますか？キャッシュは自動で再生成される一時データ（カバー検索のダウンロード、画像デコードキャッシュ、一時ファイル）です。ゲームデータとバックアップには影響しません。 | Views/SettingsView |
| `settings.cacheCleared` | 已清除缓存（释放 %@） | Cache cleared (%@ freed) | キャッシュを削除しました（%@ 解放） | Views/SettingsView |
...
| `settings.collectorModeHint` | 在游戏详情页启用「持有」页签，记录实体游戏的版本与收藏照片。 | Adds an \"Owned\" tab to game details for recording physical game editions and photos. | ゲーム詳細ページに「所持」タブを追加し、実物ゲームの版とコレクション写真を記録します。 | Views/SettingsView |
...
| `settings.hideToolbarGlass` | 隐藏上方毛玻璃 | Hide top frosted glass | 上部の毛ガラスを隠す | Views/SettingsView |
| `settings.hideToolbarGlassHint` | 隐藏所有页面（库/详情/统计）顶部工具栏的毛玻璃背景，界面更简洁。（macOS 15 及以上可启用） | Hides the top toolbar frosted-glass background on all pages (Library / Detail / Stats) for a cleaner look. (macOS 15 and later) | すべてのページ（ライブラリ / 詳細 / 統計）の上部ツールバーの毛ガラス背景を隠し、すっきりさせます。（macOS 15 以上で有効） | Views/SettingsView |
...
| `settings.keepOriginalImagesHint` | 收藏照片按原样保存、不压缩（默认压缩以节省空间；只影响之后新增的图）。 | Saves collection photos unmodified instead of compressing (applies only to images added afterwards). | コレクション写真を圧縮せずそのまま保存します（デフォルトは圧縮して容量を節約。以降に追加する写真にのみ適用）。 | Views/SettingsView |
| `settings.keyInvalid` | API Key 无效，请检查 | Invalid API key. Please check | APIキーが無効です。確認してください | Views/SettingsView |
| `settings.keyValid` | API Key 有效 | API key is valid | APIキーは有効です | Views/SettingsView |
| `settings.language` | 界面语言 | Language | 表示言語 | Views/SettingsView |
| `settings.platformIcons` | 平台标志 | Platform Logos | プラットフォームロゴ | Views/SettingsView |
| `settings.platformIconsHint` | 在各平台选择与分组中显示品牌标志图标。 | Show brand logos in platform pickers and grouping views. | プラットフォーム選択・グループ表示でブランドロゴを表示します。 | Views/SettingsView |
...
| `settings.steamGridDBHint` | 在 steamgriddb.com 免费注册后，从个人页面获取 | Register free at steamgriddb.com, then get the key from your profile | steamgriddb.com で無料登録後、プロフィールから取得 | Views/SettingsView |
...
| key | 简体中文 | English | 日本語 | 使用位置 |
|-----|---------|---------|--------|---------|
| `share.brandUser` | %@的游戏簿 | %@'s Gamelog | %@'s Gamelog | Views/SharePanelView,Share/ShareCardView |
| `share.byGames` | 按游戏分享 | By Game | ゲームで共有 | Views/SharePanelView |
| `share.byGroups` | 按分组分享 | By Group | グループで共有 | Views/SharePanelView |
| `share.count` | 共 %d 款 | %d games | 合計 %d タイトル | Share/ShareCardView |
| `share.desktop` | 电脑 (16:9) | Desktop (16:9) | PC (16:9) | Views/SharePanelView |
| `share.groupTitle` | 分组标题 | Group Title | グループタイトル | Views/SharePanelView |
| `share.multiNote` | 分享多个游戏：生成一张总览图 | Multiple selected: renders one overview image | 複数選択：1枚の総合画像を生成 | <未使用>*(动态/未命中)* |
| `share.noneSelected` | 请至少选择一个游戏 | Select at least one game | ゲームを1つ以上選択してください | Views/SharePanelView |
| `share.noneSelectedGroup` | 请至少选择一个分组 | Select at least one group | グループを1つ選択してください | Views/SharePanelView |
| `share.openShareSheet` | 分享… | Share… | 共有… | Views/SharePanelView |
| `share.overviewTitle` | 总览标题 | Overview title | タイトル | Views/SharePanelView |
| `share.overviewTitleDefault` | 我的游戏簿 | My Gamelog | My Gamelog | <未使用>*(动态/未命中)* |
| `share.phone` | 手机 (9:16) | Phone (9:16) | スマホ (9:16) | Views/SharePanelView |
...
| `share.savedToAlbum` | 已保存到相册 | Saved to Album | アルバムに保存しました | Views/SharePanelView |
...
| `share.saveToAlbum` | 保存到相册 | Save to Album | アルバムに保存 | Views/SharePanelView |
| `share.selectGames` | 选择要分享的游戏 | Select games to share | 共有するゲームを選択 | Views/SharePanelView |
| `share.singleNote` | 分享一个游戏：生成该游戏的一张卡片 | One game selected: renders a card for it | ゲームを1つ選択：そのゲームのカード画像を生成 | <未使用>*(动态/未命中)* |
...
| key | 简体中文 | English | 日本語 | 使用位置 |
|-----|---------|---------|--------|---------|
| `stats.avgScore` | 库平均分 | Average score | 平均スコア | Views/StatsView |
...
| `stats.overallRanking` | 整体排名 | Overall Ranking | 全体ランキング | Views/StatsView,Views/StatsRankings |
| `stats.prevPage` | 上一页 | Previous | 前へ | Views/StatsRankings |
| `stats.rankings` | 榜单 | Rankings | ランキング | Views/StatsView |
| `stats.totalGames` | 通关总数 | Games cleared | 総クリア数 | Views/StatsView |
...
| `status.dropped` | 弃坑 | Dropped | ギブアップ | <未使用>*(动态/未命中)* |
...
| `title.newGame` | 新建游戏 | New Game | 新しいゲーム | Views/GameEditView |

## `validation.*` 组

| key | 简体中文 | English | 日本語 | 使用位置 |
|-----|---------|---------|--------|---------|
| `validation.nameRequired` | 请输入游戏名称 | Enter the game name | ゲーム名を入力してください | Views/GameEditView |
...
| `validation.scoresRequired` | 首条记录需要完成四项评分 | The first completion needs all four scores | 初回記録には4次元のスコアが必要です | <未使用>*(动态/未命中)* |
