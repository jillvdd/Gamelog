...
- **游戏库**：网格 / 列表视图切换；按名称 + 别名 + 多语言名搜索；按状态 / 平台 / 分组筛选；分组管理（macOS 侧边栏 / iOS 筛选菜单），同一游戏可进多个分组；按名字 / 发售日期 / 通关日期 / 平均分（从低到高 / 从高到低）排序。库显示分 = 该游戏所有已评分通关记录平均分的均值，取整到 0.1；未评分显示「未评分」。
...
本仓库提供 Release 版 IPA（`dist/GameLog-beta-2.1.ipa`），未签名，需自行签名后安装。
