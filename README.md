...
一个 macOS 个人应用，用来记录你打通关的游戏：封面、六维评分、每次通关的平台/日期/程度/时长/内容，并生成适合发微博或推特的分享图。纯本地存储（SwiftData），数据完全属于你自己。
...
- **游戏详情**：评价（一句话标题 + 正文）、所有通关记录、可编辑 / 追加 / 删除；头部显示平均分 + **六维彩色条形图**（玩法 / 设计 / 剧情 / 美术 / 音乐 / 性能各一色）；窗口拉宽时评价与通关记录自动并排双栏。
- **六维评分**：玩法 / 设计 / 剧情 / 美术 / 音乐 / 性能，1–10、0.1 步进滑块。整体 = 六维均值；首条通关记录评分必填，之后的记录可勾选"本次跳过评分"。
- **通关记录**：平台、通关日期、通关程度（主线通关 / 全支线 / 全结局 / 全收集白金 / 多周目 / 速通 / 自定义）、时长、通关内容备注。
- **日期选择**：发售日期与通关日期用三列滚轮（年 / 月 / 日）选择，自动处理闰年 2 月 29 日与月末钳位。
- **封面**：本机选图，或通过 [SteamGridDB](https://www.steamgriddb.com) API 搜索并下载（需在设置里填 API Key）。搜索为即输即搜的补全式（服务端最多返回 10 条），面板可随时关闭，不必选择封面。设置里可开启**自动匹配封面**（默认关）：开 + 已配 API Key 时，新建 / 编辑游戏输入名字停顿约 0.6 秒即自动取第一条命中的竖版封面；不覆盖已有封面，失败静默。
- **个性化**：设置里可自定义用户名（20 字内）、用户头像与 app 图标（本机选图 → 裁切面板，支持拖拽平移 + 缩放；头像圆形 256²、图标圆角方形 1024²）。自定义图标即时反映到 Dock 并重启保持；侧边栏与分享图会带上你的头像。
- **分享图**：单选一张游戏 → 单卡（竖版 1080×1920 或横版 1920×1080）；多选 → 一张总览图（标题默认「{用户名}的游戏簿」、可自定义，随游戏数量自动换行）。单卡与总览格显示**最后一次通关日期**；单卡显示六维分（竖版为封面整卡背景 + 底部半透明内容条，六维以 3×2 网格展示；横版右侧信息栏）；左下角 / 左上角带品牌水印（用户名 + 圆形头像），随语言本地化。竖卡文字固定白系保证封面明暗随机下可读，配色跟随系统深浅色，可保存 PNG 或调起系统分享。
